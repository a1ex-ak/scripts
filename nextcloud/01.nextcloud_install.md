...
# 3. "Создание базы nextcloud"
	sudo mariadb
	  ...
	  SHOW DATABASES;
	  GRANT ALL PRIVILEGES ON nextcloud.* TO 'admincloud'@'localhost' IDENTIFIED BY 'k5WxDAoAIyMEOGKxISzB';
	  FLUSH PRIVILEGES;
	  exit
...
----------------------------------------------------------------
# apache2 должен установится как зависимость вместе с предыдущей командой
...
----------------------------------------------------------------
# 6. "Скачать дистрибутив Nextcloud" - https://nextcloud.com/
...
# заменить требуемые строки настроек
	...
	sudo sed -i 's/upload_max_filesize = 2M/upload_max_filesize = 3G/g' /etc/php/8.1/apache2/php.ini
	sudo sed -i 's/max_execution_time = 30/max_execution_time = 360/g' /etc/php/8.1/apache2/php.ini
	sudo sed -i 's/post_max_size = 8M/post_max_size = 3G/g' /etc/php/8.1/apache2/php.ini
